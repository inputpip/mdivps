...
SELECT 
    ...
    COALESCE(
       (
          ...
          WHERE (elem->'product'->>'id')::uuid = di.product_id 
             OR (elem->>'productId')::uuid = di.product_id
          LIMIT 1
       ), 0
    ) AS price,
    d.driver_id,
...
SELECT 
    ...
    t.customer_name,
    COALESCE((elem->'product'->>'id')::uuid, (elem->>'productId')::uuid) AS product_id,
...
SELECT 
    ...
    CASE 
        WHEN sb.helper_id IS NOT NULL AND sb.helper_id_2 IS NOT NULL AND sb.helper_id_3 IS NOT NULL 
          THEN FLOOR(COALESCE((SELECT rate_per_qty FROM commission_rules WHERE role = 'delivery_3_helpers' AND product_id = sb.product_id LIMIT 1), 0) / 4)
        WHEN sb.helper_id IS NOT NULL AND sb.helper_id_2 IS NOT NULL AND sb.helper_id_3 IS NULL 
          THEN FLOOR(COALESCE((SELECT rate_per_qty FROM commission_rules WHERE role = 'delivery_2_helpers' AND product_id = sb.product_id LIMIT 1), 0) / 3)
        WHEN sb.helper_id IS NULL 
          THEN (COALESCE((SELECT rate_per_qty FROM commission_rules WHERE role = 'driver' AND product_id = sb.product_id LIMIT 1), 0) + 
                COALESCE((SELECT rate_per_qty FROM commission_rules WHERE role = 'helper' AND product_id = sb.product_id LIMIT 1), 0))
        ELSE COALESCE((SELECT rate_per_qty FROM commission_rules WHERE role = 'driver' AND product_id = sb.product_id LIMIT 1), 0)
    END AS rate_per_qty,
    sb.branch_id
FROM sales_base sb
...
SELECT 
    ...
    CASE 
        ...
        WHEN sb.helper_id_2 IS NOT NULL AND sb.helper_id_3 IS NULL 
          THEN FLOOR(COALESCE((SELECT rate_per_qty FROM commission_rules WHERE role = 'delivery_2_helpers' AND product_id = sb.product_id LIMIT 1), 0) / 3)
        ELSE COALESCE((SELECT rate_per_qty FROM commission_rules WHERE role = 'helper' AND product_id = sb.product_id LIMIT 1), 0)
    END AS rate_per_qty,
    sb.branch_id
FROM sales_base sb
...
SELECT 
    ...
    CASE 
        WHEN sb.helper_id_3 IS NOT NULL 
          THEN FLOOR(COALESCE((SELECT rate_per_qty FROM commission_rules WHERE role = 'delivery_3_helpers' AND product_id = sb.product_id LIMIT 1), 0) / 4)
        ELSE FLOOR(COALESCE((SELECT rate_per_qty FROM commission_rules WHERE role = 'delivery_2_helpers' AND product_id = sb.product_id LIMIT 1), 0) / 3)
    END AS rate_per_qty,
    sb.branch_id
FROM sales_base sb
...
SELECT 
    ...
    'helper' AS role,
    FLOOR(COALESCE((SELECT rate_per_qty FROM commission_rules WHERE role = 'delivery_3_helpers' AND product_id = sb.product_id LIMIT 1), 0) / 4) AS rate_per_qty,
    sb.branch_id
FROM sales_base sb
...
SELECT 
    ...
    'sales' AS role,
    COALESCE((SELECT rate_per_qty FROM commission_rules WHERE role = 'sales' AND product_id = sb.product_id LIMIT 1), 0) AS rate_per_qty,
    sb.branch_id
FROM sales_base sb
...
SELECT 
    ...
    'cashier' AS role,
    COALESCE((SELECT rate_per_qty FROM commission_rules WHERE role = 'cashier' AND product_id = sb.product_id LIMIT 1), 0) AS rate_per_qty,
    sb.branch_id
FROM sales_base sb
